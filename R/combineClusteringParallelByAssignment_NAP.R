...
cellularities = list()
cellularities[[run]]=c(0.77,0.86,0.57,0.73,0.51,0.4,0.8,0.72) # NAP: can be read from FILE (cellularity)
...
for(s in 1:no.subsamples){
	data[[s]]=read.table(paste(samplenames,subsamples[[run]][s],"_allDirichletProcessInfo_fromMultipleSamplesWithoutChromosomeLosses.txt",sep=""),sep="\t",header=T)
}
#
#
library(MASS)
