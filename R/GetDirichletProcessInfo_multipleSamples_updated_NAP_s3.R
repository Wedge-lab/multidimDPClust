...
case = args[1]
run_info_path = args[2]
BB_file_path = args[3]
outpath = args[4]

Input_dir = paste0(outpath, "/", case, "/DPinput/")
...
gender = sample_info$Gender[1]
useBBfolders= sample_info$useBBfolder


no_samples = length(samplenames)
cellularity = rep(NA,no_samples)
subclone.files = rep(NA,no_samples)

for(s in 1:no_samples){
	purity_file = paste0(BB_file_path, "/", useBBfolders[s], "/" , samplenames[s], "_rho_and_psi.txt")
	cellularity[s] = read.table(purity_file,header=T)$rho[2]

	Battenberg_file=paste0(BB_file_path, "/", useBBfolders[s], "/" , samplenames[s],"_subclones.txt")

	bb = read.table(Battenberg_file,header = TRUE)
	bb$chr <- paste0("chr",bb$chr)

	write.table(bb,paste0(BB_file_path, "/", useBBfolders[s], "/" , samplenames[s],"_subclones_with_chr.txt"),sep = "\t",
	            row.names = FALSE,quote = FALSE)

	subclone.files[s] = paste0(BB_file_path, "/", useBBfolders[s], "/" , samplenames[s], "_subclones_with_chr.txt")
}
...
print(subclone.files)


sample = case
subsample <- sprintf("%02d", 1:length(samplenames))

# datafile <- paste0(samplenames,"_allDirichletProcessInfo_fromMultipleSamples.txt")
df <- data.frame(sample, subsample, samplenames, cellularity,gender)

print(df)

write.table(df, file = "sample_info.txt", sep = "\t", quote = FALSE, row.names = FALSE)


m=read.table(paste0( pickedname,"_mutCount.txt"),header=T,stringsAsFactors=F)

library(tidyr)
info=m %>% separate(ID,into=c("chr","end"),sep="\\_")
INFO=info[,1:2]
mutCount=array(m[,-1])
w=read.table(paste0(pickedname,"_WTCount.txt"),header=T,stringsAsFactors=F)
WTCount=array(w[,-1])
...
GetDirichletProcessInfo_multipleSamples <- function(samplenames, cellularity, mutCount, WTCount, subclone.files, is.male = T, out.files = NULL, phase.dir = NULL, info = INFO, SNP.phase.file = NULL, mut.phase.file = NULL, keep.muts.not.explained.by.CN=T){
	...
	for(s in 1:no.subsamples){
		...
		full.samplename = samplenames[s]
                data[[s]]$end=as.numeric(data[[s]]$end) #naser: read as character and not used in mathematical equations below -> thus NA output		
...
GetDirichletProcessInfo_multipleSamples(samplenames, cellularity, mutCount, WTCount, subclone.files, is.male = is.male, out.files = NULL, phase.dir = NULL, info = INFO, SNP.phase.file = NULL, mut.phase.file = NULL, keep.muts.not.explained.by.CN=T)
