...
######### As many functions in DPClust are not been published, we need to  
######### source the R file in DPclust package
...
if(length(args)>2){
  ...
  choose.from = as.numeric(args[3])
	choose.number = as.numeric(args[4])
	choose.index = as.numeric(args[5])
	print(choose.from)
	print(choose.number)
	print(choose.index)
...
##################The parameter you should give based on your own samples#######
...
cellularities = list()
cellularities[[run]] = as.numeric(sample_info$cellularity)
...
new_output_folder = "./05_PAoutput"
if(!file.exists(new_output_folder)){
  dir.create(new_output_folder)
}
# change this parameter accroding to your last step 
noiters=1250
# GS.data generated by last step
load(paste0( "./04_GSoutput/", case,"_gsdata.RData"))
#################################################################################
