...
print(paste(subsample.indices)) #naser: check what it outputs

sample="MR01_034_JK"

samplenames=c("PD21941")
subsamples = list()
subsamples[[run]] = c("a2","a","d","e","f","g","h","i")


cellularities = list()
cellularities[[run]]=c(0.77,0.86,0.57,0.73,0.51,0.4,0.8,0.72) # NAP: need to read this from FILE (cellularity)
...
for(s in 1:no.subsamples){
	data[[s]]=read.table(paste(samplename,subsamples[[run]][s],"_allDirichletProcessInfo_fromMultipleSamplesWithoutChromosomeLosses.txt",sep=""),sep="\t",header=T)
...
noiters=1000
conc_param=1
cluster_conc = 5 #naser: what does this represent?
...
#naser: how are these files generated?
...
load(paste0("/well/wedge/naser/melanoma/exomes/MR01_034_JK/DP_output/",sample,"_gsdata.RData")) ################################################################################################### CHANGE ACCORDINGLY#######################
S.i=GS.data$S.i
V.h=GS.data$V.h
pi.h=GS.data$pi.h 
no.subsamples = length(subsample.indices)
...
density.out = Gibbs.subclone.density.est.nd(burden=mutation.copy.number/copyNumberAdjustment,GS.data=GS.data,density.smooth=density.smooth,post.burn.in.start=burn.in+1,post.burn.in.stop=noiters,max.burden=1.5,resolution=NA)
...
if(no.optima>1){	
	...
	no.muts = nrow(mutation.copy.number) #ncol(S.i) can also be used, reverting back to original; REVISION WAS: #naser: previously was: nrow(mutation.copy.number) BUT THIS IS LOWER THAN THE NUMBER OF VARIANTS IN S.i -> gives the following error: Error in temp.preferences[S.i[s, ] == c, i] : (subscript) logical subscript too long
	...
	write.table(out,paste(new_output_folder,"/",samplename,"_",paste(subsample.indices,collapse="_"),"_DP_and_cluster_info_",density.smooth,".txt",sep=""),sep="\t",row.names=F,quote=F)
